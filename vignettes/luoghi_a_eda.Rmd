...
Function `rowMassPlot`illustrates graphically the word total frequency and the zone partition by frequency zones (quartiles at left, linguistic frequency zones at right in Figure \ref{fig:rowmass}).

```{r, eval = F}
rowMassPlot(data)
rowMassPlot(data_ling)
```
```{r rowmass, echo = F, out.width='80%', w = 6, h = 2.8, warning=track, fig.cap="Keyword total frequencies and zones based on quartiles (left) or on linguistic frequency zones (right)", fig.label="fig:rowmass", fig.show='hold', fig.pos="p", eval=T}
p1 <- rowMassPlot(data)
...
```{r colmass, echo=track, results=track, w=8, h=4, fig.cap="Subcorpora dimension: for each year, number of texts (dot-line), total number of word-tokens$/10$ in texts, column sum$/10$ in the TDM, maximum column frequency$/1$ in the TDM", fig.label="fig:colmass", fig.pos="p", out.width='100.0%', eval=T}

colMassPlot(data, sc = c(1, 50000, 400, 100), r = 2, textty = "text", themety = "light", size_b = 1.5, x_lab = "year")
