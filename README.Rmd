---
output: github_document
---

<!-- README.md is generated from README.Rmd. Please edit that file -->

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "100%",
  dpi = 300
)
```

```{r}
library(cccc)
```

```{r}
data <- importData(tdm_file = "data-raw/JPSP_TITfreq.csv",
                   corpus_file = "data-raw/JPSP_TITinfo.csv",
                   sep_tdm = ";",
                   sep_corpus_info = ";",
                   zone="stat")
```

```{r}
rowMass <- rowMassPlot(data = data)
rowMass
```

```{r}
colMass <- colMassPlot(data = data,
                         sc = c(1, 10, 10, 1),
                         r = 1,
                         textty = "text",
                         themety = "light",
                         size_b = 2.5,
                         x_lab = "years")
colMass
```

```{r}
curve <- curvePlot(data = data,
                   r = 1,
                   themety = "light",
                   size_class = NULL,
                   x_lab = "years"
)
curve
```


```{r}
curveCtu <- curveCtuPlot(data,
                         ctu_noun = c("person", "object", "instruct", "incident"),
                         r = 1,
                         themety = "light",
                         size_class = NULL,
                         x_lab = "year")
curveCtu
```

```{r}
## Normalize columns by corpus size
data_nc <- normalization(data, normty = "nc", sc = 1000)
head(data_nc$tdm, n=10)
```

```{r}
## chi-square type normalization
data_nchi <- normalization(data, normty = "nchi")
head(data_nchi$tdm, n=10)
```

```{r}
## max per row
data_nM <- normalization(data, normty = "nM")
head(data_nM$tdm, n=10)
```

```{r}
## min-max per row
data_nmM <- normalization(data, normty = "nmM")
head(data_nmM$tdm, n=10)
```

```{r}
## nonlinear normalization (based on asymmetry)
data_nnl <- normalization(data, normty = "nnl", nnlty="V")
head(data_nnl$p_asy, n=10)

head(data_nnl$tdm, n=10)
```

```{r}
curveCtu_nchi <- curveCtuPlot(data_nchi,
                              ctu_noun = c("person", "object", "instruct", "incident"),
                              r = 1,
                              themety = "light",
                              size_class = NULL,
                              x_lab = "year")
curveCtu_nchi
```

```{r}
curve_nchi <- curvePlot(data = data_nchi,
           r = 1,
           themety = "light",
           size_class = NULL,
           x_lab = "years"
)
curve_nchi
```

```{r}
facet_plot <- facetPlot(data_nchi,
   keyword_selection = list(type = "frequency", n = 3),
   r = 4,
   scales = "free")
facet_plot
```

```{r}
## Smoothing selection - P m-2
p_m2 <- smoothingSelection(data_nchi,
                                lambda_seq = NULL,
                                degrees = NULL,
                                penalty_type = "m-2",
                                plot = TRUE,
                                verbose = TRUE)
p_m2$plots
p_m2$summary_panel
```

```{r}
## Smoothing selection - P 2

p_2 <- smoothingSelection(data_nchi,
                               lambda_seq = NULL,
                               degrees = NULL,
                               penalty_type = "2",
                               normty = NULL,
                               plot = TRUE,
                               verbose = TRUE)

p_2$plots
p_2$summary_panel
```

```{r}
## Smoothing selection - P 1

p_1 <- smoothingSelection(data_nchi,
                           lambda_seq = NULL,
                           degrees = NULL,
                           penalty_type = "1",
                           plot = TRUE,
                           verbose = TRUE)

p_1$plots
p_1$summary_panel
```

```{r}
## Smoothing selection - P 0

p_0 <- smoothingSelection(data_nchi,
                           lambda_seq = NULL,
                           degrees = NULL,
                           penalty_type = "0",
                           plot = TRUE,
                           verbose = TRUE)

p_0$plots
p_0$summary_panel
```

```{r}
# Optimal Smoothing selection

resSmoothing <- list("m-2" = p_m2,
                     "2"   = p_2,
                     "1"   = p_1,
                     "0"   = p_0)

opt_res <- optimalSmoothing(resSmoothing)

# Results

opt_res$opt_summary
opt_res$m_opt
opt_res$penalty_opt
opt_res$llambda_opt
opt_res$gcv_opt
opt_res$top4_opt
```

```{r}
# Optimal Smoothing selection - Plots
opt_res$plots
```

```{r}
#######
   plot_opt <- plotOptimalSmoothing(data_nchi, opt_result = opt_res)
   plot_opt
```

```{r}
plotSubOpt <- plotOptimalFits(data=data_nchi,
                               opt_res=opt_res,
                               n_curves = 9,
                               show_zone = TRUE,
                               graph = FALSE)
plotSubOpt
```



```{r}
# Run Clustering Across a Range of Cluster Numbers
g <- runClusteringRange(data_nchi,
                         k_range = 2:26,
                         n_repeats = 20,
                         seed = 123,
                         verbose = TRUE)
```


```{r}
# Build Clustering Internal Validation Matrix (CIVf)

g2 <- buildCIVf(clustering_set=g,
                       criteria_set = NULL,
                       min_valid_values = 0.75,
                       verbose = TRUE)
g2$opt_civ
```

```{r}
#' Select Top Partitions Based on Ranking Matrix
g3 <- selectBestPartitions(g2, g, graph=T)
```

```{r}
#' Compute Adjusted Rand Index Matrix from Best Partitions
rand_result <- computeRandMatrix(g3)
head(rand_result$rand_matrix)
```


```{r}
#' Select Best Number of Clusters Based on Rand Matrix Stability

best_k <- selectBestKfromRand(rand_result)
best_k$best_k
```
```{r}
#' Extract the Best Partition for a Selected k
final_part <- extractBestPartition(g3, best_k$best_k, rand_matrix=rand_result)
final_part
```


```{r}
#' Compute Smoothed Cluster Trajectories and Summary Information
fd_obj <- computeClusterFd(data = data_nchi, final_part = final_part, opt = opt_res)
```


```{r}
#' Extract Top Keywords per Cluster by Rand Index
top_keywords <- extractTopClusterWords(fd_obj, percent_words = 50)
head(top_keywords$`1`,n=10)
head(top_keywords$`2`,n=10)

```

```{r}
#' Plot Cluster Curves from Functional Data Object
plots <- plotClusterCurvesList(fd_obj,
                               nx = 100)
plots
```

```{r}
#' Plot Cluster Trajectories
plots2 <- plotAllClusterTrajectories(fd_obj,
                            partition_id = NULL,
                            nx = 100)
plots2
```

